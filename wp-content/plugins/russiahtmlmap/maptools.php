...
function russia_html5map_plugin_detect_csv_header(&$row) {
    static $header = array(
        'states' => null,
        'groups' => null,
        'points' => null,
    ...
    if (is_null($header['states'])) {
        foreach($header as $k => $v)
            $header[$k] = russia_html5map_plugin_get_csv_import_export_keys($k);
    }
...
function russia_html5map_plugin_detect_click_action(&$params) {
...
function russia_html5map_plugin_import_csv(&$errors) {
    ...
    if (! is_uploaded_file($_FILES['csv-file']['tmp_name']))
        $errors[] = __('File upload error!', 'russia-html5-map');
    else {
        if (in_array($_FILES['csv-file']['type'], $json_types)) {
            $errors[] = sprintf(__('JSON import should be done on the "<a href="%s">Maps dashboard</a>" tab', 'russia-html5-map'), "admin.php?page=russia-html5-map-maps");
        }
        elseif (!in_array($_FILES['csv-file']['type'], $types)) {
            $errors[] = __('Wrong file type!', 'russia-html5-map');
        }
    }
    $fd = stripslashes($_REQUEST['field-delimiter']);
    $td = stripslashes($_REQUEST['text-delimiter']);
    if ( ! array_key_exists($fd, $field_delimiters)) {
         $errors[] = __('Wrong field delimiter!', 'russia-html5-map');
    }
    else {
        $fd = $field_delimiters[$fd];
    }
    if ( ! array_key_exists($td, $text_delimiters)) {
         $errors[] = __('Wrong text delimiter!', 'russia-html5-map').$td;
    }
    else {
    ...
    if ( ! $head) {
        $errors[] = __('Wrong csv file!', 'russia-html5-map');
        return false;
    }
    if ( ! ($current_type = russia_html5map_plugin_detect_csv_header($head))) {
        $errors[] = __('Wrong csv header!', 'russia-html5-map');
        return false;
    }
    $data = array(
        'states' => array(),
    ...
    while ($line = fgetcsv($fh, 0, $fd, $td)) {
        if ($new_type = russia_html5map_plugin_detect_csv_header($line)) {
    ...
    if (!empty($data['states'])) {
        ...
        foreach ($st_params as $id => &$params) {
            ...
            russia_html5map_plugin_detect_click_action($params);
        ...
        if ($st_params)
            ...
            $errors[] = __('Failed to encode new data! Probably non UTF data.', 'russia-html5-map');

        if (count($data['states'])) {
            $errors[] = __('Some data left unimported! Looks like wrong map.', 'russia-html5-map');
        }
    }
    if (!empty($data['groups'])) {
        ...
        foreach ($data['groups'] as $id => $params) {
            ...
            russia_html5map_plugin_detect_click_action($map_options['groups'][$id]);
    ...
    if (!empty($data['points'])) {
        ...
        foreach ($data['points'] as $id => $params) {
            ...
            russia_html5map_plugin_detect_click_action($map_options['points'][$id]);
    ...
    russia_html5map_plugin_save_options($all_options);
    return true;
}


if (isset($_POST['action']) && $_POST['action'] == 'russia-html5-map-import-csv') {
    if (russia_html5map_plugin_import_csv($errors))
        russia_html5map_plugin_messages(array(__('Configuration successfully imported!', 'russia-html5-map')), null);
    else
        russia_html5map_plugin_messages(null, $errors);
} elseif (isset($_POST['action']) && $_POST['action'] == 'russia-html5-map-save-js') {
    $map_options = &$options[$map_id];
    $map_options['customJs']   = stripslashes($_POST['custom_js']);
    $map_options['update_time'] = time();
    russia_html5map_plugin_save_options($options);
    russia_html5map_plugin_messages(array(__('Custom js successfully updated!', 'russia-html5-map')), null);
}

echo "<div class=\"wrap russia-html5-map main full\"><h2>" . __('Tools', 'russia-html5-map') . "</h2>";
?>
<script>
jQuery(function(){

    jQuery('.russia-html5-map-acs-label').click(function() {
        jQuery(this)
            .toggleClass('russia-html5-map-closed')
            .next().filter('.russia-html5-map-acs').toggleClass('russia-html5-map-closed');
    });
    ...
    jQuery('input[name=csv-file]').change(function() {
        ...
        if (this.checkValidity && !this.checkValidity())
            return;
        jQuery('form[name=action_form_import] input[name=action]').val('russia-html5-map-import-csv');
...
<?php 
    russia_html5map_plugin_map_selector('tools', $map_id, $options);
    echo "<br /><br />\n";
    russia_html5map_plugin_nav_tabs('tools', $map_id);
?>
    <h3><?php _e('Batch export / import from CSV', 'russia-html5-map') ?></h3>
    <input type="submit" class="button button-secondary export" value="<?php esc_attr_e('Export to CSV file', 'russia-html5-map'); ?>" />&nbsp;&nbsp;&nbsp;
    <input type="button" class="button button-secondary csv-import" value="<?php esc_attr_e('Import from CSV file', 'russia-html5-map'); ?>" />
    <br/><br/>
    <label class="russia-html5-map-acs-label russia-html5-map-closed"><?php _e('Advanced CSV settings', 'russia-html5-map'); ?></label>
    <div class="russia-html5-map-acs  russia-html5-map-closed" style="padding-bottom: 10px">
        <label class="title"><?php _e('Field delimiter:', 'russia-html5-map') ?></label> <select name="field-delimiter" style="width: 100px">
            <option value=",">,</option>
            <option value=";">;</option>
            <option value=":">:</option>
            <option value="sp"><?php _e('{SPACE}', 'russia-html5-map') ?></option>
            <option value="tb"><?php _e('{TAB}', 'russia-html5-map') ?></option>
        </select><br>
        <label class="title"><?php _e('Text delimiter:', 'russia-html5-map') ?></label> <select name="text-delimiter"  style="width: 100px">
            <option value='"'>"</option>
            <option value="'">'</option>
        </select>
    ...
    <br>
    <p><?php printf(__('Note that this tool exports/imports area data only. If you need to make a full backup of all settings of the map including general settings, use the backup option provided on <a href="%s">"Maps"</a> page.', 'russia-html5-map'), '?page=russia-html5-map-maps'); ?></p>
    <p class="help"><?php _e('* The term "area" means one of the following: continent, country, state, province, county or district, depending on the particular plugin.', 'russia-html5-map'); ?></p>
    <input type="hidden" name="action" value="russia-html5-map-export-csv">
    </form>

    <br><br>
    <form method="POST" class="" name="action_form_js" enctype="multipart/form-data">
    <h3><?php _e('Custom JavaScript', 'russia-html5-map') ?></h3>
    <p><?php _e('Here you can add any custom javascript to extend plugin\'s functionality', 'russia-html5-map') ?></p>
    <label class="russia-html5-map-acs-label russia-html5-map-closed"><?php _e('Detailed description', 'russia-html5-map'); ?></label>
    <div class="russia-html5-map-acs  russia-html5-map-closed">
        <p><?php _e('Any code placed here will be executed after the map is drawn.', 'russia-html5-map') ?></p>
        <p><?php _e('Here are the list of plugin-related variables, that will be available for you:', 'russia-html5-map') ?><ul>
        <li><b>map</b> - <?php _e('Map instance. All available features are listed <a href="https://docs.html5maps.com/html5-locator-maps/api">here</a>.', 'russia-html5-map') ?></li>
        <li><b>containerId</b> - <?php _e('HTML id of the container that contains the map.', 'russia-html5-map') ?></li>
        <li><b>mapId</b> - <?php _e('Id of the map, specified in shortcode.', 'russia-html5-map') ?></li>
        </ul>
        <p><?php _e('You can find out any id of the area or point by viewing the CSV export.', 'russia-html5-map'); ?></p>
        <h4>Example:</h4>
        <pre class="code">
<span class="v">map</span>.<span class="m">on</span>(<span class="s">'click'</span>, <span class="k">function</span> (<span class="v">event</span>, <span class="v">sid</span>, <span class="v">map</span>) {
    ...
    <input type="hidden" name="action" value="russia-html5-map-save-js" />
