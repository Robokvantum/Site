...
function russia_html5map_plugin_popup_bulder_is_available() {
    return (function_exists('sgRegisterScripts') or class_exists('\sgpb\PopupLoader'));
}

function russia_html5map_plugin_popup_bulder_type() {
...
function russia_html5map_plugin_popup_builder_can_enable_scripts(&$mapOptions) {
    if ( ! russia_html5map_plugin_popup_bulder_is_available())
        return false;
...
function russia_html5map_plugin_popup_builder_queue_popups($mapPopupIds = array()) {
...
function russia_html5map_plugin_popup_builder_enqueue_popups_to_output($popups) {
    return array_merge($popups, russia_html5map_plugin_popup_builder_queue_popups());
}

function russia_html5map_plugin_popup_builder_enable_scripts(&$mapOptions, $parseStates) {
    if ( !russia_html5map_plugin_popup_builder_can_enable_scripts($mapOptions))
        return false;

    switch (russia_html5map_plugin_popup_bulder_type()) {
        case 2:
            $popupIds = array();
            ...
            if ($popupIds) {
                russia_html5map_plugin_popup_builder_queue_popups($popupIds);

                if (!$hooksRegistered) {
                    add_filter('sgpbLoadablePopups', 'russia_html5map_plugin_popup_builder_enqueue_popups_to_output');
...
function russia_html5map_plugin_popup_builder_list_available_popups() {
    global $wpdb;
    $popups  = array();
    if (class_exists('\sgpb\PopupLoader')) {
...
function russia_html5map_plugin_popup_builder_cover_old_ids(&$mapOptions) {
