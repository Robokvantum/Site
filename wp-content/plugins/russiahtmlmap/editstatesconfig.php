...
if(isset($_POST['act_type']) && $_POST['act_type'] == 'russia-html5-map-states-save') {
    ...
    if ($sId) {
        ...
        if(isset($_POST['URL'][$vals['id']]))
            $states[$s_id]['link'] = russia_html5map_plugin_chk_url(stripslashes($_POST['URL'][$vals['id']]));
        ...
        if(isset($_POST['color'][$vals['id']]))
            $states[$s_id]['color_map'] = russia_html5map_plugin_chk_color($_POST['color'][$vals['id']]);

        if(isset($_POST['color_'][$vals['id']]))
            $states[$s_id]['color_map_over'] = russia_html5map_plugin_chk_color($_POST['color_'][$vals['id']]);
        ...
        foreach($states as $k=>$v) {
            $states[$k]['color_map'] = russia_html5map_plugin_chk_color($_POST['color'][$vals['id']]);
        }
        }

        if(isset($_POST['colorOverCh'][$vals['id']])) {
        foreach($states as $k=>$v) {
            $states[$k]['color_map_over'] = russia_html5map_plugin_chk_color($_POST['color_'][$vals['id']]);
        }
        }
...
echo "<div class=\"wrap russia-html5-map main full\"><h2>" . __('Configuration of Map Areas', 'russia-html5-map') . "</h2>";
?>
<script>
    var map_cfg = {
...
?>
    ...
    jQuery(function($){
        ...
        jQuery('select[name=state_select]').change(function() {
            location.href='admin.php?page=russia-html5-map-states&map_id=<?php echo $map_id; ?>&s_id='+jQuery(this).val();
        });
...
<?php
    ...
    russia_html5map_plugin_map_selector('states', $map_id, $options);
    echo "<br /><br />\n";
    russia_html5map_plugin_nav_tabs('states', $map_id);

?>

    <p><?php echo __('This tab allows you to add the area-specific information and adjust colors of individual area on the map.', 'russia-html5-map'); ?></p>
    <p class="help"><?php echo __('* The term "area" means one of the following: region, state, country, province, county or district, depending on the particular plugin.', 'russia-html5-map'); ?></p>

    <select name="state_select" class="bulk-hide">
        <option value=0><?php echo __('Select an area', 'russia-html5-map'); ?></option>
        <?php
    ...
    <button type="button" class="button button-secondary map-show" style="margin-top: 1px" data-toggle="<?php esc_attr_e('Hide map', 'russia-html5-map'); ?>"><?php _e('Choose on map', 'russia-html5-map') ?></button>

    <button class="button button-secondary bulk-selectall notbulk-hide" style="margin-top: 1px"><?php _e('Toggle selections', 'russia-html5-map') ?></button>

    <?php if ($isbulk) { ?>
        <a href="admin.php?page=russia-html5-map-states&map_id=<?php echo $map_id; ?>" class="button button-secondary bulk-editing-classic" style="margin-top: 1px"><?php _e('Back to one-by-one editing', 'russia-html5-map') ?></a>
    <?php } else { ?>
        <a href="admin.php?page=russia-html5-map-states&map_id=<?php echo $map_id; ?>&bulk" class="button button-secondary bulk-editing bulk-hide" style="margin-top: 1px"><?php _e('Bulk Edit', 'russia-html5-map') ?></a>
    <?php } ?>
    ...
    <fieldset>
        <legend><?php echo __('Map area', 'russia-html5-map'); ?></legend>

        <div style="" id="stateinfo-<?php echo $vals['id']?>" class="stateinfo">
        <span class="title"><?php echo __('Name:', 'russia-html5-map'); ?> </span><input class="" type="text" name="name[<?php echo $vals['id']?>]" value="<?php echo $vals['name']?>" />
        <span class="tipsy-q" original-title="<?php esc_attr_e('Name of Area', 'russia-html5-map'); ?>">[?]</span>
        ...
        <?php echo __('do not show popup name', 'russia-html5-map'); ?>
        </label>
        <div class="clear"></div>

        <span class="title"><?php echo __('Shortname:', 'russia-html5-map'); ?> </span><input class="" type="text" name="shortname[<?php echo $vals['id']?>]" value="<?php echo $vals['shortname']?>" />
        <span class="tipsy-q" original-title="<?php esc_attr_e('Shortname of Area. Use \n to break the lines.', 'russia-html5-map'); ?>">[?]</span>
        <div class="clear"></div>

        <span class="title"><?php echo __('What to do when the area is clicked:', 'russia-html5-map'); ?></span>
        <label><input type="radio" name="URLswitch[<?php echo $vals['id']?>]" id="n<?php echo $vals['id']?>" value="nill" <?php echo $rad_nill?> autocomplete="off">&nbsp;<?php echo __('Nothing', 'russia-html5-map'); ?></label> <span class="tipsy-q" original-title="<?php esc_attr_e('Do not react on mouse clicks', 'russia-html5-map'); ?>">[?]</span>&nbsp;&nbsp;&nbsp;
        <label><input type="radio" name="URLswitch[<?php echo $vals['id']?>]" id="u<?php echo $vals['id']?>" value="url" <?php echo $rad_url?> autocomplete="off">&nbsp;<?php echo __('Open a URL', 'russia-html5-map'); ?></label> <span class="tipsy-q" original-title="<?php esc_attr_e('A click on this area opens a specified URL', 'russia-html5-map'); ?>">[?]</span>&nbsp;&nbsp;&nbsp;
        <label><input type="radio" name="URLswitch[<?php echo $vals['id']?>]" id="m<?php echo $vals['id']?>" value="more" <?php echo $rad_more?> autocomplete="off">&nbsp;<?php echo __('Show more info', 'russia-html5-map'); ?></label> <span class="tipsy-q" original-title="<?php esc_attr_e('Displays a side-panel with additional information (contacts, addresses etc.)', 'russia-html5-map'); ?>">[?]</span>&nbsp;&nbsp;&nbsp;
        <label><input type="radio" name="URLswitch[<?php echo $vals['id']?>]" id="p<?php echo $vals['id']?>" value="popup-builder" <?php echo $rad_popup?> autocomplete="off" <?php echo (!count($popups)) ? "disabled" : ""; ?>>&nbsp;<?php echo __('Show lightbox popup', 'russia-html5-map'); ?></label> <span class="tipsy-q" original-title="<?php esc_attr_e('Show lightbox popup, that you are can create with the plugin "Popup Builder". To activate this option, install the "Popup Builder" plugin, then switch to it and create a new popup there. Then, you should open this tab again and specify the name of the created popup here.', 'russia-html5-map'); ?>">[?]</span><br />
        <div style="<?php echo $style_input; ?>" id="stateURL<?php echo $vals['id']?>">
            <span class="title"><?php echo __('URL:', 'russia-html5-map'); ?> </span><input style="width: 240px;" class="" type="text" name="URL[<?php echo $vals['id']?>]" id="URL<?php echo $vals['id']?>" value="<?php echo $vals['link']?>" />
            <span class="tipsy-q" original-title="<?php esc_attr_e('The landing page URL', 'russia-html5-map'); ?>">[?]</span>&nbsp;&nbsp;&nbsp;
            <label><input type="checkbox" name="isNewWindow[<?php echo $vals['id']?>]" <?php if (!empty($vals['isNewWindow'])) echo 'checked="checked" '; ?>/> <?php echo __('Open url in a new window', 'russia-html5-map'); ?></label></br>
        </div>
        ...
        <?php russia_html5map_plugin_wp_editor_for_tooltip($vals['comment'], "info[{$vals['id']}]", 'info'); ?>
        <br />
        <span class="title"><?php echo __('Area color:', 'russia-html5-map'); ?> </span><input class="color colorSimple" type="text" name="color[<?php echo $vals['id']?>]" value="<?php echo $vals['color_map']?>" style="background-color: #<?php echo $vals['color_map']?>"  />
        <span class="tipsy-q" original-title='<?php esc_attr_e('The color of an area.', 'russia-html5-map'); ?>'>[?]</span><div class="colorpicker"></div>
        <label><input name="colorSimpleCh[<?php echo $vals['id']?>]" class="colorSimpleCh" type="checkbox" /> <?php echo __('Apply to all areas', 'russia-html5-map'); ?></label>
        <br />
        <span class="title"><?php echo __('Area hover color:', 'russia-html5-map'); ?> </span><input class="color colorOver" type="text" name="color_[<?php echo $vals['id']?>]" value="<?php echo $vals['color_map_over']?>" style="background-color: #<?php echo $vals['color_map_over']?>"  />
        <span class="tipsy-q" original-title='<?php echo __('The color of an area when the mouse cursor is over it.', 'russia-html5-map'); ?>'>[?]</span><div class="colorpicker"></div>
        <label><input name="colorOverCh[<?php echo $vals['id']?>]" class="colorOverCh" type="checkbox" /> <?php echo __('Apply to all areas', 'russia-html5-map'); ?></label>
        <br />
        ...
        <span class="title"><?php echo __('CSS class:', 'russia-html5-map'); ?> </span>
        <input class="" type="text" id="class-<?php echo $vals['id']?>" name="class[<?php echo $vals['id']?>]" value="<?php echo isset($vals['class']) ? $vals['class'] : '' ?>" />
        <span class="tipsy-q" original-title="<?php esc_attr_e('You can specify several CSS classes separated by space', 'russia-html5-map'); ?>">[?]</span>
        <label style="margin-left:20px; cursor: default"><span style="color: #666">ID: <?php echo 'st'.$sId ?> </span><span class="tipsy-q" original-title="<?php esc_attr_e('Use this ID when interacting with the map via API', 'russia-html5-map'); ?>">[?]</span></label>
        <br />

        <span class="title"><?php echo __('Hide area:', 'russia-html5-map'); ?> </span>
        <label><input name="hidden[<?php echo $vals['id']?>]" type="radio" value="show" <?php if (!isset($vals['hidden'])) echo 'checked="checked"'; ?> /> <?php echo __('show', 'russia-html5-map'); ?></label>
        <label><input name="hidden[<?php echo $vals['id']?>]" type="radio" value="hide" <?php if (isset($vals['hidden'])) echo 'checked="checked"'; ?> /> <?php echo __('hide', 'russia-html5-map'); ?></label>
        <br />
        </div>
    ...
    <input type="hidden" name="act_type" value="russia-html5-map-states-save" />
    <p class="submit"><input type="submit" value="<?php esc_attr_e('Save Changes', 'russia-html5-map'); ?>" class="button-primary" id="submit" name="submit" disabled="disabled"></p>
</form>
</div>
